...
<div class=" {showPeopleListValue!=0 ? "w-5/6": "w-full"} h-full  transition-all duration-200 ease-linear bg-[#36393f] flex flex-col">
    ...
    <div class="  w-full  py-2 px-1 flex flex-row h-14 place-items-center  ">
        ...
        <button class=" hover:bg-gray-800 hover:rounded-xl ">
            ...
            <input  autocomplete="off" bind:value="{messengerValue}" on:keypress="{keyHandler}" type="text" name="string" placeholder="Type Message" class="  min-w-fit grow   self-center h-12 w-[415px] p-2 text-lg font-medium text-[#afb0b4] outline-0 focus:border-sky-600  bg-[#303338] border-2 ml-2 my-2 border-[#24262b]  active:border-gray-800 rounded-2xl  " on:focus="{()=>{writeFocus=true}}" on:blur="{()=>{writeFocus = false}}">
