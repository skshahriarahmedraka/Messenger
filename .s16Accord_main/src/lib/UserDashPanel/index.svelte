...
{#if showUserOptions}
	 <div class=" h-80 w-full mb-0 bg-[#18191c] bg-opacity-80 text-gray-300   rounded-xl flex flex-row flex-wrap "  on:outclick={onClickOutside}  on:blur={()=>{ showUserOptions =false}} >
		<div class="w-full h-20">
			<img src={MyPro.CoverImage} alt="coverImage" class="h-40 w-full object-cover rounded-xl">
			<img src={MyPro.ProfileImage} alt="coverImage" class="h-12 w-12 -mt-7 ml-5  object-cover rounded-xl">
			<p class="">{MyPro.Name}</p>
		</div> 

		<button on:click={()=>{}} class=" w-1/2 rounded-lg hover:bg-gray-900 ">Profile</button>
		 <button on:click={()=>{}} class=" "> Settings</button>
			...
			<div class="flex flex-col  ">
				...
				{:else}
					<p
						...
						class="{setTimeout(() => { showUserid=true }, 1000)} cursor-pointer text-sm text-blue-500 line-clamp-1 "
